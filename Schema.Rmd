...
# Add SKU_ID column
SKU$SKU_ID <- seq_len(nrow(SKU)) + 10000

# Remove unrealistic cases
SKU <- SKU[!(SKU$CATEGORY == 'Dress' & SKU$GENDER == 'Male'), ]
SKU <- SKU[!(SKU$PRODUCT_NAME == 'Bra' & SKU$GENDER == 'Male'), ]

SKU$PRODUCT_SELLING_PRICE <- round(runif(nrow(SKU),min=20,max=100),2)
SKU$PRODUCT_SELLING_PRICE[SKU$CATEGORY == "Accessories"] <- round(runif(sum(SKU$CATEGORY == "Accessories"), min = 5, max = 20), 2)
markup <- runif(nrow(SKU), min = 0.1, max = 0.3)
SKU$PRODUCT_PURCHASING_PRICE <- round(SKU$PRODUCT_SELLING_PRICE /(1 + markup), 2)
```
