...
ORDER <- data.frame(
  PRODUCT_ID = character(1000),
  stringsAsFactors = TRUE
)

# Sampling products from SKU 
ORDER$PRODUCT_ID <- sample(SKU$PRODUCT_ID, size = 1000, replace = TRUE)

# Add ORDER_QUANTITY
ORDER$ORDER_QUANTITY <-round(runif(nrow(ORDER),min=1,max=5),2)

# Add ORDER_STATUS:
status <- c("Order placed","In transit","Delivered","Cancelled","Returned")
probabilities <- c(0.1, 0.35, 0.5, 0.025, 0.025)
ORDER$ORDER_STATUS <- as.factor(sample(status, size = nrow(ORDER), replace = TRUE,prob = probabilities))
...
# Add ORDER_ID column
ORDER$ORDER_ID <- paste0("OD", seq_len(nrow(ORDER)) + 10000)
```
