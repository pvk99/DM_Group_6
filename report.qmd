...
**Github Link: <https://github.com/pvk99/DM_Group_6>**
...
![](images/clipboard-2548644980.png)
...
-   CUSTOMERS refers CUSTOMERS: A self-recursive relationship, in which one existing customer can refer many new customer, but one new customer can only be refered by one existing customer.

![](images/clipboard-2193275668.png){width="300"}
...
![](images/clipboard-2646036484.png){width="300"}
...
![](images/clipboard-4120801883.png){width="300"}
...
![](images/clipboard-2157952684.png){width="465"}
...
## the number of rows in CUSTOMER, generate additional names
...
for (i in 1:nrow(ORDER)) {
  ...
  product_ids <- 
  sample(SKU$PRODUCT_ID, size = products_per_order[i], replace = FALSE)
...
ifelse(ORDER$ORDER_STATUS=="Returned", 
...
# Define a function to check if phone numbers are of length 10 
...
for (table_name in names(tables)) {
  for (file_path in file_paths[[table_name]]) {
    ...
    if (table_name == "ADS") {
      ...
      "CLICK_THROUGH_RATE", "NUMBER_OF_CLICK")
      for (attr in numeric_attrs) {
        if (any(table_data[[attr]] < 0)) {
          error_list <- c(error_list, 
          paste("Negative values found in", attr, 
          "column of ADS table."))
    ...
    if (table_name == "CUSTOMERS") {
      ...
      for (i in 1:nrow(table_data)) {
        if (!validate_phone_numbers(table_data$PHONE_NUMBER[i])) {
          error_list <- 
          c(error_list, 
          paste("Invalid phone number in CUSTOMERS table:", table_data$PHONE_NUMBER[i]))
    ...
    if (table_name == "ORDERS") {
      ...
      for (attr in numeric_attrs) {
        if (any(!is.na(table_data[[attr]]) & table_data[[attr]] < 0)) {
          error_list <- c(error_list, 
          paste("Invalid or negative values found in", attr, "column of ORDERS table."))
          invalid_rows <- c(invalid_rows, 
          which(!is.na(table_data[[attr]]) & table_data[[attr]] < 0))
        }
      }
    ...
    for (i in seq(nrow(table_data))) {
      ...
      if (any(is.na(pk_values))) {
        error_list <- c(error_list, 
        paste("Null primary key value found in", table_name, "table."))
        next  # Skip to the next record if primary key is null
      }
      
      conditions <- paste(pk_columns, "=", paste0("'", pk_values, "'"), 
      collapse = " AND ")
      ...
      if (key_exists == 0) {
        tryCatch({
          ...
          error_list <- c(error_list, 
          paste("Error inserting record with primary key", 
          paste(pk_values, collapse = ", "), "into table", table_name))
          print(paste("Error inserting record with primary key", 
          paste(pk_values, collapse = ", "), "into table", table_name))
          print(e)
        })
      } else {
...
MARKUP * PRODUCT_PURCHASING_PRICE + PRODUCT_PURCHASING_PRICE 
AS SELLING_PRICE 
FROM SKU ) T2 
on T1.PRODUCT_ID = T2.PRODUCT_ID"
...
Short_Term_Plot1 <- ggplot(top_10_SKUs, aes(x = reorder(name_sku, REVENUE), y = SALES)) +
  ...
  labs(x = "Product Names", y = "Units Sold", title = "Top 10 Most Sold SKUs in Units in Last 30 days", subtitle = "Labels indicate revenue generated by the SKUs") +
  coord_flip() +
  theme(axis.text.x = element_text(hjust = 1)) + 
...
( SELECT SKU.PRODUCT_ID AS PRODUCT_ID,
         ...
         SKU.SIZE AS SIZE,
         SKU.COLOR AS COLOR,
      SKU.PRODUCT_PURCHASING_PRICE * (1 + SKU.MARKUP) 
...
FROM
    ( SELECT ORDERS.PRODUCT_ID AS PRODUCT_ID,
      ...
      julianday(ORDERS.ORDER_DATE) AS INTEGER ) <= 365
      GROUP BY ORDERS.PRODUCT_ID
...
LEFT JOIN
  ...
  SELECT ADS.PLATFORM AS PLATFORM,
       ...
       CASE
        ...
        julianday(ADS.AD_START_DATE) AS INTEGER ) <= 90 THEN 'Last 90 days'
        WHEN cast(julianday('now') - 
        julianday(ADS.AD_START_DATE) AS INTEGER ) BETWEEN 90 AND 180 THEN '90 to 180 days'
        WHEN cast(julianday('now') - 
        julianday(ADS.AD_START_DATE) AS INTEGER ) BETWEEN 180 AND 270 THEN '180 to 270 days'
        WHEN cast(julianday('now') - 
        julianday(ADS.AD_START_DATE) AS INTEGER ) BETWEEN 270 AND 365 THEN '270 to 365 days'
        ELSE 'More than one year'
       END AS QUARTER_AD
  FROM   ADS
  WHERE  cast(julianday('now') - 
  julianday(ADS.AD_START_DATE) AS INTEGER ) <= 365
...
SELECT CUSTOMERS.CUSTOMER_ID AS CUSTOMER_ID,
        ...
        ((cast(julianday('now') - 
        julianday(CUSTOMERS.DATE_OF_BIRTH) AS INTEGER))/365)  AS AGE_YEARS
  FROM    CUSTOMERS
  WHERE AD_ID IS NOT NULL
) AS T1
...
![](images/clipboard-3836582107.png){width="476"}

### A2. LLM Prompt Sequence

![](images/clipboard-819306190.png)

![](images/clipboard-3421991236.png)

![](images/clipboard-816743079.png)

![![](images/clipboard-4078166433.png)](images/clipboard-775462801.png)

![](images/clipboard-3654311040.png)

![](images/clipboard-3477003729.png)

![![](images/clipboard-1098269934.png)](images/clipboard-3477003729.png)

![![](images/clipboard-1159365385.png)![](images/clipboard-1987729016.png)](images/clipboard-242529755.png)

![![](images/clipboard-1458403283.png)](images/clipboard-585330277.png)

![![](images/clipboard-3051402899.png)](images/clipboard-2467015082.png)

![![](images/clipboard-147268864.png)![](images/clipboard-59606176.png)](images/clipboard-4290294969.png)

![![](images/clipboard-2071520630.png)](images/clipboard-2742179981.png)

![![](images/clipboard-3382278366.png)![](images/clipboard-3105468105.png)](images/clipboard-1658987171.png)

![![](images/clipboard-3168498792.png)![](images/clipboard-2022032540.png)![](images/clipboard-1229439777.png)![](images/clipboard-2858169503.png)![](images/clipboard-2421636128.png)![](images/clipboard-3807993522.png)![](images/clipboard-3730510066.png)![](images/clipboard-3238198266.png)](images/clipboard-1569235788.png)
