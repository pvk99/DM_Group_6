...
# CREATE HOUSE_NUMBER
CUSTOMER$HOUSE_NUMBER <- round(runif(nrow(CUSTOMER), min = 1, max = 50),0)

# CREATE AD_ID
CUSTOMER$AD_ID_1 <- sample(ADS$AD_ID, size = nrow(CUSTOMER), replace = TRUE)
CUSTOMER$AD_ID <- ifelse(CUSTOMER$ACQUISITION_TYPE=="Paid",CUSTOMER$AD_ID_1,"")
CUSTOMER$AD_ID_1 <- NULL
#Save to csv 
