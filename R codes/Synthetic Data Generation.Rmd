...
#Save to csv file
write.csv(CATEGORY, file = file.path("/cloud/project/Data","CATEGORY.csv"), row.names = FALSE)
```
...
#Save to csv 
write.csv(SUPPLIER, file = file.path("/cloud/project/Data","SUPPLIER.csv"), row.names = FALSE)
```
...
#Save to csv 
write.csv(SKU, file = file.path("/cloud/project/Data","SKU.csv"), row.names = FALSE)
```
...
#Save to csv file
write.csv(PROMOTION, file = file.path("/cloud/project/Data","PROMOTION.csv"), row.names = FALSE)
```
...
ADS$AD_START_DATE <- as.Date(sort(sample(seq(ad_start, ad_end, by = "day"), nrow(ADS), replace = TRUE)))
ADS$AD_END_DATE <- as.Date(ADS$AD_START_DATE + sample(10,nrow(ADS),replace=TRUE))
...
#Save to csv 
write.csv(ADS, file = file.path("/cloud/project/Data","ADS.csv"), row.names = FALSE)
```
...
CUSTOMER$ACQUISITION_TYPE[1:5] <- sample(c("Paid", "Organic"), size = 5, replace = TRUE)
...
CUSTOMER$DATE_OF_BIRTH <- as.Date(sample(seq(start_DOB, end_DOB, by = "day"), nrow(CUSTOMER), replace = TRUE))
...
# CREATE HOUSE_NUMBER
CUSTOMER$HOUSE_NUMBER <- as.character(round(runif(nrow(CUSTOMER), min = 1, max = 50),0))

# CREATE AD_ID
CUSTOMER$AD_ID <- ifelse(CUSTOMER$ACQUISITION_TYPE=="Paid", sample(ADS$AD_ID, size = nrow(CUSTOMER), replace = TRUE),"")
#Save to csv 
write.csv(CUSTOMER, file = file.path("/cloud/project/Data","CUSTOMERS.csv"), row.names = FALSE)
```
...
#Save to csv 
write.csv(ORDER_SHIPMENT, file = file.path("/cloud/project/Data","ORDER_SHIPMENT.csv"), row.names = FALSE)
```
...
#Save to csv 
write.csv(TRANSACTION, file = file.path("/cloud/project/Data","TRANSACTIONS.csv"), row.names = FALSE)
```
...
# Add ORDER_STATUS:
ORDER$ORDER_STATUS <- as.factor(ifelse(ORDER$ORDER_DATE<="2024-03-01",sample(c("Delivered","Returned"), replace=TRUE, prob = c(0.92,0.8)),sample(c("Order placed","In transit"), replace=TRUE, prob = c(0.1,0.9))))

# Add SHIPPING_ID
ORDER$SHIPPING_ID <- ORDER_SHIPMENT$SHIPPING_ID

# Add DELIVERY_DATE
ORDER$DELIVERY_DATE <- ifelse(ORDER$ORDER_STATUS %in% c("Delivered","Returned"),as.Date(ORDER$ORDER_DATE + sample(1:7,nrow(ORDER),replace=TRUE)),"")
...
# Add RETURN_DATE
ORDER$RETURN_DATE <- ifelse(ORDER$ORDER_STATUS=="Returned", as.Date(ORDER$ORDER_DATE + sample(1:30,nrow(ORDER),replace=TRUE)),"")

#Save to csv 
write.csv(ORDER, file = file.path("/cloud/project/Data","ORDERS.csv"), row.names = FALSE)
```
